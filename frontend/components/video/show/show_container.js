...
const mapStateToProps = (state, ownProps) => {
  ...
  const comments = Object.keys(state.entities.comments).map((id) => state.entities.comments[id]);
  for (let i = 0; i < comments.length; i++) {
    comments[i].authorName = state.entities.users[comments[i].author_id].username;
  }
